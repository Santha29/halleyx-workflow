...
def wf_to_dict(wf, include_steps=False):
    ...
    if include_steps:
        d["steps"] = [step_to_dict(s, include_rules=True) for s in wf.steps]
    return d
...
def step_to_dict(s, include_rules=False):
    ...
    if include_rules:
        d["rules"] = [rule_to_dict(r) for r in s.rules]
    return d
...
def exec_to_dict(e, include_logs=False):
    dur = None
    ...
    if include_logs:
        d["logs"] = [log_to_dict(l) for l in e.logs]
    return d

def log_to_dict(l):
    return {"id": l.id, "step_id": l.step_id, "step_name": l.step_name,
            "step_type": l.step_type, "status": l.status,
            "rule_evaluated": l.rule_evaluated, "next_step_id": l.next_step_id,
            "next_step_name": l.next_step_name, "message": l.message,
            "approver_email": l.approver_email, "duration_ms": l.duration_ms,
...
def list_workflows(name: Optional[str] = None, isActive: Optional[bool] = None,
    ...
    return ok({"content": [wf_to_dict(w) for w in items], "totalElements": total,
               "totalPages": (total + size - 1) // size, "page": page})
...
def update_workflow(wf_id: str, req: WorkflowReq):
    ...
    wf.input_schema = req.input_schema; wf.is_active = req.is_active
...
def create_step(wf_id: str, req: StepReq):
    ...
    order = req.step_order or (max([s.step_order for s in existing], default=0) + 1)
...
def update_step(step_id: str, req: StepReq):
    ...
    if req.metadata_json: s.metadata_json = req.metadata_json
...
def run_workflow(execution: Execution, wf: Workflow, input_data: dict, db: Session):
    ...
    while cur:
        if iterations >= MAX_ITERATIONS:
            raise Exception(f"Max loop iterations ({MAX_ITERATIONS}) reached. Infinite loop detected!")
        iterations += 1
        ...
        if cur.step_type == "APPROVAL":
            ...
            msg_parts.append(f"Approval processed. Approver: {approver or 'N/A'}")
        elif cur.step_type == "NOTIFICATION":
            ch = meta.get("notification_channel", "email")
            tmpl = meta.get("template", "")
            msg_parts.append(f"Notification sent via {ch}" + (f". Template: {tmpl}" if tmpl else ""))
        elif cur.step_type == "TASK":
            instr = meta.get("instructions", "")
            msg_parts.append(f"Task executed." + (f" {instr}" if instr else ""))
        ...
        log = StepLog(id=gen_uuid(), execution_id=execution.id,
                      step_id=cur.id, step_name=cur.name, step_type=cur.step_type,
                      status="COMPLETED", rule_evaluated=matched_rule.condition if matched_rule else None,
                      next_step_id=next_step_id, next_step_name=next_step_name,
                      message=". ".join(msg_parts) + f" | {rule_msg}",
...
def execute_workflow(wf_id: str, req: ExecutionReq = None):
    ...
    db.refresh(ex)
    return ok(exec_to_dict(ex, include_logs=True), "Executed")
...
def list_executions(workflowId: Optional[str] = None, status: Optional[str] = None,
    ...
    return ok({"content": [exec_to_dict(e) for e in items], "totalElements": total,
               "totalPages": (total + size - 1) // size, "page": page})
...
def get_execution(exec_id: str):
    ...
    return ok(exec_to_dict(ex, include_logs=True))
...
def cancel_execution(exec_id: str):
    ...
    ex = db.query(Execution).filter(Execution.id == exec_id).first()
    if not ex: err("Execution not found", 404)
    if ex.status not in ["IN_PROGRESS", "PENDING"]: err("Cannot cancel execution in status: " + ex.status)
    ex.status = "CANCELED"; ex.ended_at = datetime.utcnow()
    ex.error_message = "Cancelled by user"; db.commit()
    return ok(exec_to_dict(ex, include_logs=True), "Cancelled")
...
def retry_execution(exec_id: str):
    ...
    try:
        ...
        ex.ended_at = datetime.utcnow(); db.commit()
    db.refresh(ex)
    return ok(exec_to_dict(ex, include_logs=True), "Retried")
...
def seed():
    ...
    s1 = Step(id=gen_uuid(), workflow_id=wf1.id, name="Manager Approval", step_type="APPROVAL", step_order=1,
              metadata_json='{"assignee_email":"manager@company.com","instructions":"Review expense"}')
    ...
    s3 = Step(id=gen_uuid(), workflow_id=wf1.id, name="CEO Approval", step_type="APPROVAL", step_order=3,
              metadata_json='{"assignee_email":"ceo@company.com"}')
    s4 = Step(id=gen_uuid(), workflow_id=wf1.id, name="Task Completion", step_type="TASK", step_order=4,
              metadata_json='{"instructions":"Mark expense approved, notify requester"}')
    s5 = Step(id=gen_uuid(), workflow_id=wf1.id, name="Task Rejection", step_type="TASK", step_order=5,
              metadata_json='{"instructions":"Mark expense rejected, notify requester"}')
    db.add_all([s1, s2, s3, s4, s5]); db.flush()
    wf1.start_step_id = s1.id

    db.add_all([
        Rule(id=gen_uuid(), step_id=s1.id, priority=1, condition="amount > 100 && country == 'US' && priority == 'High'", next_step_id=s2.id, description="High value US → Finance"),
        Rule(id=gen_uuid(), step_id=s1.id, priority=2, condition="amount <= 100", next_step_id=s4.id, description="Low value → Complete"),
        ...
        Rule(id=gen_uuid(), step_id=s2.id, priority=1, condition="amount > 10000", next_step_id=s3.id, description="Very high → CEO"),
        Rule(id=gen_uuid(), step_id=s2.id, priority=2, condition="DEFAULT", next_step_id=s4.id, description="Default → Complete"),
        Rule(id=gen_uuid(), step_id=s3.id, priority=1, condition="DEFAULT", next_step_id=s4.id, description="CEO approved"),
        Rule(id=gen_uuid(), step_id=s4.id, priority=1, condition="DEFAULT", next_step_id=None, description="END"),
        Rule(id=gen_uuid(), step_id=s5.id, priority=1, condition="DEFAULT", next_step_id=None, description="END"),
    ])
    ...
    db.add_all([
        ...
        Rule(id=gen_uuid(), step_id=t3.id, priority=1, condition="DEFAULT", next_step_id=None, description="END"),
    ])
    db.commit(); db.close()
